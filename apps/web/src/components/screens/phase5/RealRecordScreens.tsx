      ...
      <div className="mx-auto w-full max-w-[1100px] space-y-3 p-3 md:p-4">
        ...
        <div className="grid gap-3 md:grid-cols-[minmax(0,1fr)_320px]">
          <Panel title={t15("summary")}>
            <EmptyState title={t15("noSourceOccurrences")} reason={t15("sourceBacked")} />
          </Panel>
...
export function RealWitnessScreen({ witness }: { witness: Witness }) {
  const t = useTranslations("phase5");
  const tb = useTranslations("phase5b");
  const t15 = useTranslations("phase15");
  ...
  return (
    <AppShell crumbs={[{ label: tb("witnesses"), href: "/witnesses" }, { label: witness.code }]}>
      ...
      />
      <div className="mx-auto grid w-full max-w-[1100px] gap-3 p-3 md:grid-cols-[minmax(0,1fr)_320px] md:p-4">
        <Panel title={t("testimony")}>
          <EmptyState title={t15("noWitnessContent")} reason={t15("sourceBacked")} />
        </Panel>
